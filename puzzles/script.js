...
class PuzzleGame {
    ...
    configurerCanvas() {
        ...
        if (this.estMobile()) {
            largeurEcran = Math.min(window.innerWidth - 40, 400);
            
            this.canvasPieces.width = largeurEcran;
            this.canvasPieces.height = largeurEcran * 1.7; // Ratio pour 3 rangées
            this.canvasResolution.width = largeurEcran;
            this.canvasResolution.height = largeurEcran * 1.5; // Ratio pour 3 rangées
            
            this.pieceWidth = largeurEcran / 2;
            this.pieceHeight = this.pieceWidth;
        } else {
            this.canvasPieces.width = 450;
            this.canvasPieces.height = 600;
            this.canvasResolution.width = 400;
            this.canvasResolution.height = 600;
            
            this.pieceWidth = 200;
            this.pieceHeight = 200;
        }
        
        // Ajuste le canvas d'overlay à la taille du conteneur
        this.canvasOverlay.width = this.zonesJeuEl.clientWidth;
        this.canvasOverlay.height = this.zonesJeuEl.clientHeight;
    }
    ...
    initialiserPuzzle() {
        ...
        // Calcule les positions de la grille de départ
        this.calculerPositionsGrillePieces();
        
        const largeurPieceAffichage = 150;
        const hauteurPieceAffichage = 150;
        ...
        for (let row = 0; row < 3; row++) {
            for (let col = 0; col < 2; col++) {
                this.pieces.push({
                    col: col,
                    row: row,
                    sx: col * (this.imageActive.width / 2),
                    sy: row * (this.imageActive.height / 3),
                    sWidth: this.imageActive.width / 2,
                    sHeight: this.imageActive.height / 3,
                    x: 0, // Sera défini par mélangerPieces
                    y: 0, // Sera défini par mélangerPieces
                    positionInitialeIndex: 0, // Sera défini par mélangerPieces
                    width: largeurPieceAffichage,
                    height: hauteurPieceAffichage,
                    placée: false,
                    positionFinaleX: col * this.pieceWidth,
                    positionFinaleY: row * this.pieceHeight
                });
                index++;
            }
        }
        
        this.mélangerPieces(); // Mélange les pièces à l'initialisation
    }
